...
import org.azertio.plugins.db.ConnectionParameters;
import org.azertio.plugins.db.DataSet;
import org.azertio.plugins.db.DbEngine;
...
public class JooqDbEngine implements DbEngine, AutoCloseable {
	...
	private DSLContext dslContext(String alias) {
		...
		if (dslContext == null) {
			throw new AzertioException("Unknown datasource {}",alias);
		}
		return dslContext;
	}
	...
	public void insertXLSFile(String datasource, String table, java.nio.file.Path xlsFile) {
		try {
			...
			throw new AzertioException(e, "Failed to load Excel file {}", xlsFile);
		}
	}
	...
	public List<DataSet> readXls(Path file) {
		try {
			...
			throw new AzertioException(e, "Failed to read Excel file {}", file);
		}
	}
