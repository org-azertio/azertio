...
class HtmlOutputTest {
    ...
    private static final Path OUTPUT_DIR = Path.of("target/test-output/html");

    @Test
    void generatesSampleHtmlReport() throws Exception {
        ...
        Config config = Config.ofMap(Map.of(
            ...
            "htmlreport.outputDir",          OUTPUT_DIR.toAbsolutePath().toString(),
            "htmlreport.title",              "Sample REST API Report",
            "htmlreport.includePassedSteps", "true",
            "htmlreport.logoPath",           logoPath.toAbsolutePath().toString(),
            "htmlreport.accentColor",        "#1A4A6A",
            "htmlreport.footer",             "Confidential - ACME Corporation - Internal use only"
        ));
        ...
        execNodeWithMessage(execRepo, execID, sB3, ExecutionResult.FAILED, t.plusMillis(ms + 260), 45,
            "AssertionError: expected status 401 but got 200\n  at HttpAssert.assertStatus(HttpAssert.java:42)\n  at StepRunner.run(StepRunner.java:118)");
        ...
        execNodeWithMessage(execRepo, execID, sD2, ExecutionResult.ERROR, t.plusMillis(ms + 55), 440,
            "java.net.ConnectException: Connection refused: /users/9999\n  Caused by: java.io.IOException: No route to host");
        ...
        execNodeWithMessage(execRepo, execID, sOS4, ExecutionResult.FAILED, t.plusMillis(ms + 390), 25,
            "Expected status 422 but got 200. Response body: {\"orderId\":\"ORD-9999\",\"status\":\"PENDING\"}");
        ...
        execNodeWithMessage(execRepo, execID, sFC3, ExecutionResult.FAILED, t.plusMillis(ms + 465), 50,
            "Expected status 200 but got 500. Server returned: Internal Server Error");
        ...
        Path html = Files.list(OUTPUT_DIR)
            .filter(p -> p.getFileName().toString().endsWith(".html"))
            .max(Comparator.comparingLong(p -> p.toFile().lastModified()))
            .orElseThrow(() -> new AssertionError("No HTML file found in " + OUTPUT_DIR));

        assertThat(html.toFile().length()).isGreaterThan(0);

        String content = Files.readString(html);
        assertThat(content).startsWith("<!DOCTYPE html>");
        assertThat(content).contains("Sample REST API Report");
        assertThat(content).contains("class=\"badge failed\"");
        assertThat(content).contains("class=\"badge error\"");
        assertThat(content).contains("class=\"badge passed\"");
        assertThat(content).contains("Authentication");
        assertThat(content).contains("User Management");
        assertThat(content).contains("Order Management");
        assertThat(content).contains("Product Catalog");
        assertThat(content).contains("Notifications");
        assertThat(content).contains("AssertionError: expected status 401 but got 200");
        assertThat(content).contains("Expected status 422 but got 200");
        assertThat(content).contains("ACME Corporation");
        assertThat(content).contains("step-msg");

        System.out.println("\n>>> HTML report: " + html.toAbsolutePath() + "\n");
    }
    ...
    private UUID execNodeWithMessage(TestExecutionRepository repo, UUID execID, UUID planNodeID,
                                     ExecutionResult result, Instant start, long durationMs, String message) {
        UUID id = repo.newExecutionNode(execID, planNodeID);
        repo.updateExecutionNodeStart(id, start);
        repo.updateExecutionNodeFinish(id, result, start.plusMillis(durationMs));
        repo.updateExecutionNodeMessage(id, message);
        return id;
    }
}